...
updateplots(new_file_path="/Users/tiffnk/Desktop/DNAoutput/T12_8_1_2016.csv",
		most_recent_master_dataset="/Users/tiffnk/Desktop/DNAoutput/emptyMaster.RData", 
...
updateplots <- function(new_file_path, most_recent_master_dataset, plot_output_folderpath, master_dataset_output_folderpath, width, height){
	...
	if(filename%in%data_list$all$original_filename){
		...
		calc_copies <- function(DNAtype, Cq){
			if(DNAtype=="T3"){
				copies <- 6.79412433e10*exp(-0.534115977*Cq)
				detection_limit <- 79412433e10*exp(-0.534115977*40)
			}else if (DNAtype=="T4"){
				copies <- 3.72752038e11*exp(-0.620879002*Cq)
				detection_limit <- 3.72752038e11*exp(-0.620879002*40)
			}else if (DNAtype=="T10"){
				copies <- 9.63166711e10*exp(-0.572888354*Cq)
				detection_limit <- 9.63166711e10*exp(-0.572888354*40)
			}else if (DNAtype=="T11"){
				copies <- 3.52125544e11*exp(-0.647606141*Cq)
				detection_limit <- 3.52125544e11*exp(-0.647606141*40)
			}else if (DNAtype=="T12"){
				copies <- 1.68987401e11*exp(-0.597594122*Cq)
				detection_limit <- 1.68987401e11*exp(-0.597594122*40)
			}
			return(copies)
		}
		output$copies <- NA
		...
		if(n=="STOP"){
			...
			plots["MSAS_copies_date_plot"] <- tryCatch({list(ggplot(data=data_list$MSAS,aes(x=Sample_date, y=copies_abv_detection, color=duplicate, group=duplicate)) +
					...
					xlab("\nDatetime") + ylab("Copies\n") + ggtitle("DNA") + facet_wrap(~DNAtype, ncol=1, scales="free_y" ))},
			...
			plots["MSASdupdupplotT4"] <-  tryCatch({list(ggplot(data=MSAST4, aes(x=copies_abv_detection.x, y=copies_abv_detection.y)) +geom_point()+
								...
								xlab("\nCopies (Duplicate 2)")+ylab("Copies (Duplicate 1)\n")+ ggtitle(paste(DNAtype, "r2=",  rsq(MSAST4), sep="")))},
					error = function(e) list(NULL))
			plots["MSASdupdupplotT10"] <-  tryCatch({list(ggplot(data=MSAST10, aes(x=copies_abv_detection.x, y=copies_abv_detection.y)) +geom_point()+
								...
								xlab("\nCopies (Duplicate 2)")+ylab("Copies (Duplicate 1)\n")+ ggtitle(paste(DNAtype, "r2=",  rsq(MSAST10), sep="")))},
					error = function(e) list(NULL))
			plots["MSASdupdupplotT11"] <-  tryCatch({list(ggplot(data=MSAST11, aes(x=copies_abv_detection.x, y=copies_abv_detection.y)) +geom_point()+
								...
								xlab("\nCopies (Duplicate 2)")+ylab("Copies (Duplicate 1)\n")+ ggtitle(paste(DNAtype, "r2=",  rsq(MSAST11), sep="")))},
					error = function(e) list(NULL))
			plots["MSASdupdupplotT12"] <-  tryCatch({list(ggplot(data=MSAST12, aes(x=copies_abv_detection.x, y=copies_abv_detection.y)) +geom_point()+
								...
								xlab("\nCopies (Duplicate 2)")+ylab("Copies (Duplicate 1)\n")+ ggtitle(paste(DNAtype, "r2=",  rsq(MSAST12), sep="")))},
					error = function(e) list(NULL))
			...
			plots["LYS_copies_date_plot"] <- tryCatch({list(ggplot(data=data_list$LYS,aes(x=Sample_date, y=copies_abv_detection, color=duplicate, group=duplicate)) +
					 geom_line()+geom_point(aes(shape=Sample_type), size=6) +facet_wrap(~LYS_num, ncol=3, scales="free_y")+
		...
		plots["LYSdupdupplotT3"] <-  tryCatch({list(ggplot(data=LYST3, aes(x=copies_abv_detection.x, y=copies_abv_detection.y)) +geom_point()+
							geom_smooth(method="lm", show.legend=TRUE) +
					 		...
					 		xlab("\nCopies (Duplicate 2)")+ylab("Copies (Duplicate 1)\n")+ ggtitle(DNAtype)+facet_wrap(~LYS_num, ncol=3, scales="free_y"))},
		 		error = function(e) list(NULL))
 ...
 ########
 updateplots_soil <- function(new_file_path, most_recent_master_dataset, plot_output_folderpath, master_dataset_output_folderpath, width, height){
	 library(ggplot2)
	 load(most_recent_master_dataset)
	 filelocation <- new_file_path
	 output <- read.csv(filelocation)
	 ...
	 if(filename%in%data_list$all$original_filename){
		 ...
		 calc_copies <- function(DNAtype, Cq){
			 if(DNAtype=="T3Soil"){
				 ...
				 detection_limit <- 7e10*exp(-0.534*40)
			 }else if (DNAtype=="T4Soil"){
				 copies <- 4e10*exp(-0.621*Cq)
				 detection_limit <- 4e10*exp(-0.621*40)
			 }else if (DNAtype=="T10Soil"){
				 copies <- 1e11*exp(-0.573*Cq)
				 detection_limit <- 1e11*exp(-0.573*40)
			 }else if (DNAtype=="T11Soil"){
				 copies <- 4e11*exp(-0.648*Cq)
				 detection_limit <- 4e11*exp(-0.648*40)
			 }else if (DNAtype=="T12Soil"){
		 ...
		 if(n=="STOP"){
			 ...
			 MSAST3Soil <- data_list$MSASduplicates[which(data_list$MSASduplicates$DNAtype.x=="T3Soil"),]
			 MSAST4Soil <- data_list$MSASduplicates[which(data_list$MSASduplicates$DNAtype.x=="T4Soil"),]
			 MSAST10Soil <- data_list$MSASduplicates[which(data_list$MSASduplicates$DNAtype.x=="T10Soil"),]
			 MSAST11Soil <- data_list$MSASduplicates[which(data_list$MSASduplicates$DNAtype.x=="T11Soil"),]
			 MSAST12Soil <- data_list$MSASduplicates[which(data_list$MSASduplicates$DNAtype.x=="T12Soil"),]
			 
			 LYST3Soil <- data_list$LYSduplicates[which(data_list$LYSduplicates$DNAtype.x=="T3Soil"),]
			 LYST4Soil <- data_list$LYSduplicates[which(data_list$LYSduplicates$DNAtype.x=="T4Soil"),]
			 LYST10Soil <- data_list$LYSduplicates[which(data_list$LYSduplicates$DNAtype.x=="T10Soil"),]
			 LYST11Soil <- data_list$LYSduplicates[which(data_list$LYSduplicates$DNAtype.x=="T11Soil"),]
			 LYST12Soil <- data_list$LYSduplicates[which(data_list$LYSduplicates$DNAtype.x=="T12Soil"),]
			 
			 
			 plots["MSASdupdupplotT3Soil"] <-  tryCatch({list(ggplot(data=MSAST3Soil, aes(x=copies_abv_detection.x, y=copies_abv_detection.y)) +geom_point()+
										 geom_smooth(method="lm", show.legend=TRUE) +
										 geom_text(x = 1500, y = 3500, label = lm_eqn(MSAST3Soil), parse = TRUE)+
										 xlab("\nCopies (Duplicate 2)")+ylab("Copies (Duplicate 1)\n")+ ggtitle(DNAtype))},
					 error = function(e) list(NULL))
			 plots["MSASdupdupplotT4Soil"] <-  tryCatch({list(ggplot(data=MSAST4Soil, aes(x=copies_abv_detection.x, y=copies_abv_detection.y)) +geom_point()+
										 geom_smooth(method="lm", show.legend=TRUE) +
										 geom_text(x = 1500, y = 3500, label = lm_eqn(MSAST4Soil), parse = TRUE)+
										 xlab("\nCopies (Duplicate 2)")+ylab("Copies (Duplicate 1)\n")+ ggtitle(DNAtype))},
					 error = function(e) list(NULL))
			 plots["MSASdupdupplotT10Soil"] <-  tryCatch({list(ggplot(data=MSAST10Soil, aes(x=copies_abv_detection.x, y=copies_abv_detection.y)) +geom_point()+
										 geom_smooth(method="lm", show.legend=TRUE) +
										 geom_text(x = 1500, y = 3500, label = lm_eqn(MSAST10Soil), parse = TRUE)+
										 xlab("\nCopies (Duplicate 2)")+ylab("Copies (Duplicate 1)\n")+ ggtitle(DNAtype))},
					 error = function(e) list(NULL))
			 plots["MSASdupdupplotT11Soil"] <-  tryCatch({list(ggplot(data=MSAST11Soil, aes(x=copies_abv_detection.x, y=copies_abv_detection.y)) +geom_point()+
										 geom_smooth(method="lm", show.legend=TRUE) +
										 geom_text(x = 1500, y = 3500, label = lm_eqn(MSAST11Soil), parse = TRUE)+
										 xlab("\nCopies (Duplicate 2)")+ylab("Copies (Duplicate 1)\n")+ ggtitle(DNAtype))},
					 error = function(e) list(NULL))
			 plots["MSASdupdupplotT12Soil"] <-  tryCatch({list(ggplot(data=MSAST12Soil, aes(x=copies_abv_detection.x, y=copies_abv_detection.y)) +geom_point()+
										 geom_smooth(method="lm", show.legend=TRUE) +
										 geom_text(x = 1500, y = 3500, label = lm_eqn(MSAST12Soil), parse = TRUE)+
										 xlab("\nCopies (Duplicate 2)")+ylab("Copies (Duplicate 1)\n")+ ggtitle(DNAtype))},
					 error = function(e) list(NULL))
			 ...
			 plots["LYSdupdupplotT3Soil"] <-  tryCatch({list(ggplot(data=LYST3Soil, aes(x=copies_abv_detection.x, y=copies_abv_detection.y)) +geom_point()+
										 geom_smooth(method="lm", show.legend=TRUE) +
										 geom_text(x = 1500, y = 3500, label = lm_eqn(LYST3Soil), parse = TRUE)+
										 xlab("\nCopies (Duplicate 2)")+ylab("Copies (Duplicate 1)\n")+ ggtitle(DNAtype)+facet_wrap(~LYS_num, ncol=3))},
					 error = function(e) list(NULL))
			 plots["LYSdupdupplotT4Soil"]  <-  tryCatch({list(ggplot(data=LYST4Soil, aes(x=copies_abv_detection.x, y=copies_abv_detection.y)) +geom_point()+
										 geom_smooth(method="lm", show.legend=TRUE) +
										 geom_text(x = 1500, y = 3500, label = lm_eqn(LYST4Soil), parse = TRUE)+
										 xlab("\nCopies (Duplicate 2)")+ylab("Copies (Duplicate 1)\n")+ ggtitle(DNAtype)+facet_wrap(~LYS_num, ncol=3))},
					 error = function(e) list(NULL))
			 plots["LYSdupdupplotT10Soil"]  <-  tryCatch({list(ggplot(data=LYST10Soil, aes(x=copies_abv_detection.x, y=copies_abv_detection.y)) +geom_point()+
										 geom_smooth(method="lm", show.legend=TRUE) +
										 geom_text(x = 1500, y = 3500, label = lm_eqn(LYST10Soil), parse = TRUE)+
										 xlab("\nCopies (Duplicate 2)")+ylab("Copies (Duplicate 1)\n")+ ggtitle(DNAtype)+facet_wrap(~LYS_num, ncol=3))},
					 error = function(e) list(NULL))
			 plots["LYSdupdupplotT11Soil"]  <-  tryCatch({list(ggplot(data=LYST11Soil, aes(x=copies_abv_detection.x, y=copies_abv_detection.y)) +geom_point()+
										 geom_smooth(method="lm", show.legend=TRUE) +
										 geom_text(x = 1500, y = 3500, label = lm_eqn(LYST11Soil), parse = TRUE)+
										 xlab("\nCopies (Duplicate 2)")+ylab("Copies (Duplicate 1)\n")+ ggtitle(DNAtype)+facet_wrap(~LYS_num, ncol=3))},
					 error = function(e) list(NULL))
			 plots["LYSdupdupplotT12Soil"]  <-  tryCatch({list(ggplot(data=LYST12Soil, aes(x=copies_abv_detection.x, y=copies_abv_detection.y)) +geom_point()+
										 geom_smooth(method="lm", show.legend=TRUE) +
										 geom_text(x = 1500, y = 3500, label = lm_eqn(LYST12Soil), parse = TRUE)+
										 xlab("\nCopies (Duplicate 2)")+ylab("Copies (Duplicate 1)\n")+ ggtitle(DNAtype)+facet_wrap(~LYS_num, ncol=3))},
					 error = function(e) list(NULL))
